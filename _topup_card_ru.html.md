...
# Пополнение баланса QIWI Кошелька {#payment_wallet}

Операция используется для перевода средств с агентского счета на счет Клиента в системе QIWI Wallet.

Контрагент должен указывать тип средств, которые получены Контрагентом от клиента – наличные или безналичные средства (см. параметр запроса `income_wire_transfer`).
