...
Если сервер QIWI Wallet не смог обработать запрос на пополнение карты, API возвращает ответ с кодом произошедшей ошибки. **В этом случае информация о транзакции отсутствует в ответе, поэтому вы должны перейти к [запросам статуса](#status), не отклоняя платеж на своей стороне**.
