	...
	<auth>
		<payment>
			...
			<to>
				...
				<extra name="account0">id:2;acc:7988111****;sum:115.00</extra>
			</to>
		</payment>
	</auth>
</request>
...
*amount* | Сумма к перечислению провайдеру
*service-id* | Идентификатор сервиса, на который производится зачисление средств при выплате (константа: `29130`)
*account-number* | Идентификатор Клиента в системе QIWI Wallet (совпадает с номером мобильного телефона).
*extra name="account0"* | Описание транзитной транзакции (в формате *параметр1:значение;параметр2:значение;..*). Должны быть указаны параметры:<br>`id` - идентификатор провайдера, на которого будет создана транзитная транзакция (определяется [отдельным запросом](#get-provider));<br>`acc` - номер мобильного телефона Клиента;<br>`sum` - сумма платежа.
*ccy* | Валюта учетной записи Клиента. Опциональный параметр. В случае его передачи проверяется наличие у Клиента учетной записи в данной валюте. В качестве значения используется цифровой или буквенный код валюты по ISO 4217.
...
*amount* | Сумма к зачислению на счет мобильного телефона.|Отсутствуют.
*service-id* | Идентификатор сервиса, на который производится зачисление средств при выплате.|Отсутствуют.
*account-number* | Номер мобильного телефона Клиента.|Отсутствуют.
...
Если сервер не смог обработать запрос на пополнение баланса учетной записи Клиента в системе QIWI Wallet/пополнение мобильного телефона, он возвращает XML-ответ с описанием произошедшей ошибки.
...
<aside class="notice">Данные запросы должны отправляться на URL https://api.qiwi.com/xml/xmlcp.jsp. При <a href="#ssl-auth">авторизации по сертификату</a> данные запросы должны отправляться на URL https://private-api.qiwi.com/xml/xmlcp.jsp.
</aside>
...
*terminal-id* | Идентификатор агента в системе QIWI Wallet
*extra name="password"* | Экстра-поле, содержащее пароль для аутентификации агента в системе QIWI Wallet
*phonenumber* | Номер телефона Клиента, по которому нужно определить id провайдера
